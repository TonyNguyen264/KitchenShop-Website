...
public class AdminController {
	...
	public String index(HttpServletRequest request, Model model) {
		...
		ResponseEntity<DoanhSo[]> list = rt.getForEntity("http://localhost:9000/doanhso", DoanhSo[].class);
			...
			if (u.getIDQuyen() == 1) {
				if (u.getUsername().equalsIgnoreCase(username)) {
					if (u.getPassword().equalsIgnoreCase(password)) {
						...
						rt = new RestTemplate();
						ResponseEntity<NhanVien[]> listn = rt.getForEntity("http://localhost:9000/nhanvien",
								NhanVien[].class);
