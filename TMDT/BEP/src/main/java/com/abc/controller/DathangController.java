...
public class DathangController extends GetList {
	...
	public ModelAndView addgiohang(Model model, HttpServletRequest request, GioHang gh) {
		...
		for (GioHang g : listgh) {
			if (g.getIDSP() == gh.getIDSP()) {
				...
				params.put("IDSP", String.valueOf(gh.getIDSP()));
				rt.put("http://localhost:9000/giohang/" + Username + "/{IDSP}", gh, params);
		...
		rt = new RestTemplate();
		boolean kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/giohang", gh, String.class));
		updateGioHang(request);
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.setViewName("redirect:./sanpham");
		return modelAndView;
	...
	public String xoasp(@PathVariable("IDSP") int IDSP, HttpServletRequest request) {
		...
		String Username = tk.getUsername();
		rt.delete("http://localhost:9000/xoagiohang/" + Username + "/{IDSP}", params);
		updateGioHang(request);
	...
	public String lichsumuahang(HttpServletRequest request, Model model) {
		...
		if (k == null) {
			...
			NhanVien nv = rt.getForObject("http://localhost:9000/nhanvientk/{Username}", NhanVien.class, params);
	...
	public String xacnhandathang(DonHang dh, Model model, HttpServletRequest request, String Email) {
		...
		TaiKhoan tk = (TaiKhoan) adsession.getAttribute("login");
		params.put("Username", tk.getUsername());
		KhachHang khach = rt.getForObject("http://localhost:9000/khachhangtk/{Username}", KhachHang.class, params);
		// if (tk.getIDQuyen() == 2) {
		if (khach.getIDKH() == 0) {
			...
			rt = new RestTemplate();
			kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/newkhachhang", khach, String.class));

		}
		if (tk.getIDQuyen() == 2) {
			params = new HashMap<>();
			params.put("Username", tk.getUsername());
			khach = rt.getForObject("http://localhost:9000/khachhangtk/{Username}", KhachHang.class, params);
		} else {
			khach = rt.getForObject("http://localhost:9000/getKhachHang_IDKH", KhachHang.class);
			params.put("Username", tk.getUsername());
			NhanVien nv = rt.getForObject("http://localhost:9000/nhanvientk/{Username}", NhanVien.class, params);
		...
		rt = new RestTemplate();
		if (tk.getIDQuyen() == 2)
			kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/donhang", dh, String.class));
		else
			kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/donhangbynhanvien", dh, String.class));

		// lấy IDDH vừa thêm
		dh = rt.getForObject("http://localhost:9000/getDonHang_IDDH", DonHang.class);
		// Thêm chitietdh từ list gio hàng của user
		params.put("Username", tk.getUsername());
		ResponseEntity<GioHang[]> listg = rt.getForEntity("http://localhost:9000/giohang/{Username}", GioHang[].class,
				params);
		...
		for (GioHang gh : listgh) {
			...
			ct.setGia(gh.getGia());
			rt = new RestTemplate();
			kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/themchitietdh", ct, String.class));
		}
		// xóa giohang theo user
		params.put("Username", tk.getUsername());
		rt.delete("http://localhost:9000/xoagiohang/{Username}", params);
		updateGioHang(request);
