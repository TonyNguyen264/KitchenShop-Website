...
public class NhanvienController extends GetList {
	...
	public String addnhanvien(NhanVien nv) {
		RestTemplate rt = new RestTemplate();
		kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/nhanvien", nv, String.class));
	...
	public String dangki(HttpServletRequest request, TaiKhoan tk, Model model, HttpSession session) {
		...
		rt.put("http://localhost:9000/tknhanvien/{IDNV}", tk.getUsername(), params);
	...
	public String deletenhanvien(@PathVariable("IDNV") int IDNV) {
		...
		NhanVien nv = rt.getForObject("http://localhost:9000/nhanvien/{IDNV}", NhanVien.class, params);
		rt.delete("http://localhost:9000/nhanvien/{IDNV}", params);
		if (nv.getUsername() != "") {
			rt = new RestTemplate();
	...
	public String editnhanvien(@PathVariable("IDNV") int IDNV, NhanVien nv) {
		...
		Map<String, String> params = new HashMap<>();
		params.put("IDNV", String.valueOf(IDNV));
		rt.put("http://localhost:9000/nhanvien/{IDNV}", nv, params);
		return "redirect:../ds_nhanvien";
	}
}
