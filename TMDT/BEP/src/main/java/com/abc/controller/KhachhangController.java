...
public class KhachhangController {
	...
	public String addkhachhang(KhachHang kh) {
		RestTemplate rt = new RestTemplate();
		boolean kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/khachhang", kh, String.class));
	...
	public String dangki(Model model) {
		 ...
		 * rt.getForEntity("http://localhost:9000/taikhoan", TaiKhoan[].class);
		 * TaiKhoan[] listtk = list.getBody(); model.addAttribute("listk", listtk);
		 * 
		 * ResponseEntity<KhachHang[]> listn =
		 * rt.getForEntity("http://localhost:9000/khachhang", KhachHang[].class);
	...
	public String dangkikh(HttpServletRequest request, TaiKhoan tk, Model model, HttpSession session) {
		...
		boolean kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/dangki", tk, String.class));

		Map<String, String> params = new HashMap<>();
		params.put("IDKH", String.valueOf(n.getIDKH()));
		rt.put("http://localhost:9000/tkkhachhang/{IDKH}", tk.getUsername(), params);
	...
	public String deletekhachhang(@PathVariable("IDKH") int IDKH) {
		...
		KhachHang kh = rt.getForObject("http://localhost:9000/khachhang/{IDKH}", KhachHang.class, params);
		rt.delete("http://localhost:9000/khachhang/{IDKH}", params);
		if (kh.getUsername() != "") {
			rt = new RestTemplate();
			params.put("Username", kh.getUsername());
			rt.delete("http://localhost:9000/taikhoanxu/{Username}", params);
		}
