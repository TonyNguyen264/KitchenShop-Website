...
public class HomeController extends GetList {
	...
	public String getsanphamtheoloai(HttpServletRequest request, @PathVariable("IDLoai") int IDLoai, Model model) {
		...
		ResponseEntity<SanPham[]> list = rt.getForEntity("http://localhost:9000/dssanpham/{IDLoai}", SanPham[].class,
				params);
	...
	public String luachon(luachon l, Model model) {
		...
		if (l.getLc() == 1) {
			rt = new RestTemplate();
			ResponseEntity<SanPham[]> list1 = rt.getForEntity("http://localhost:9000/sanphammoinhat", SanPham[].class);
			SanPham[] lists = list1.getBody();
			model.addAttribute("listsp", lists);
		} else if (l.getLc() == 2) {
			rt = new RestTemplate();
			ResponseEntity<SanPham[]> list1 = rt.getForEntity("http://localhost:9000/sanphamview", SanPham[].class);
			SanPham[] lists = list1.getBody();
			model.addAttribute("listsp", lists);
		} else if (l.getLc() == 3) {
			rt = new RestTemplate();
			ResponseEntity<SanPham[]> list1 = rt.getForEntity("http://localhost:9000/sanphamsale", SanPham[].class);
			SanPham[] lists = list1.getBody();
			model.addAttribute("listsp", lists);
		} else
	...
	public String getidsanpham(HttpServletRequest request, @PathVariable("IDSP") int IDSP, Model model) {
		...
		SanPham sp = rt.getForObject("http://localhost:9000/sanpham/{IDSP}", SanPham.class, params);
		...
		ResponseEntity<SanPham[]> listspn = rt.getForEntity("http://localhost:9000/sanphamngaunhien", SanPham[].class);
		...
		params.put("IDSP", String.valueOf(IDSP));
		ResponseEntity<DanhGia[]> listd = rt.getForEntity("http://localhost:9000/danhgia/{IDSP}", DanhGia[].class,
				params);
		...
		if (adsession.getAttribute("login") != null) {
			...
			for (int i = 0; i < listDH.length; i++) {
				...
				ResponseEntity<ChiTietDH[]> listct = rt.getForEntity("http://localhost:9000/chitietdh/{IDDH}",
	...
	public String themdanhgia(Model model, HttpServletRequest request, DanhGia dg) {
		...
		dg.setNgayDG(date);
		RestTemplate rt = new RestTemplate();
		rt.delete("http://localhost:9000/danhgia/" + dg.getIDKH() + "/" + dg.getIDSP());
		kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/danhgia", dg, String.class));
	...
	public String dangnhap(String Username, String Password, HttpServletRequest request, Model model) {
		RestTemplate rt = new RestTemplate();
		ResponseEntity<TaiKhoan[]> list = rt.getForEntity("http://localhost:9000/taikhoan", TaiKhoan[].class);
	...
	public String addthongtinkhachhang(HttpServletRequest request, KhachHang kh, Model model) {
		RestTemplate rt = new RestTemplate();
		ResponseEntity<KhachHang[]> list = rt.getForEntity("http://localhost:9000/khachhang", KhachHang[].class);
		...
		rt = new RestTemplate();
		kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/khachhang", kh, String.class));
	...
	public String dangkithongtin(HttpServletRequest request, Model model) {
		...
		if (tk.getIDQuyen() == 1) {
			ResponseEntity<NhanVien[]> list = rt.getForEntity("http://localhost:9000/nhanvien", NhanVien[].class);
	...
	public String dangkikhachhang(HttpServletRequest request, TaiKhoan tk, String Email, Model model) {
		...
		String sub = "XÁC NHẬN ĐĂNG KÍ TÀI KHOẢN BEPSHOP";
		...
		rt = new RestTemplate();
		kq = Boolean.parseBoolean(rt.postForObject("http://localhost:9000/dangki", tk, String.class));
		RestTemplate rtp = new RestTemplate();
		kq = Boolean.parseBoolean(rtp.postForObject("http://localhost:9000/dkikhachhang", k, String.class));
	...
	public String suakh(@PathVariable("IDKH") int IDKH, Model model) {
		...
		KhachHang kh = rt.getForObject("http://localhost:9000/khachhang/{IDKH}", KhachHang.class, params);
	...
	public String suakhachhang(Model model, HttpServletRequest request, @PathVariable("IDKH") int IDKH, KhachHang kh) {
		...
		Map<String, String> params = new HashMap<>();
		params.put("IDKH", String.valueOf(IDKH));
		rt.put("http://localhost:9000/khachhang/{IDKH}", kh, params);
		return "redirect:../thongtinkhachhang";
	}
	...
	public String quenmatkhau(String Email, HttpServletRequest request, Model model) {
		...
		for (KhachHang kh : listkh) {
			if (kh.getEmail().equalsIgnoreCase(Email)) {
				...
				TaiKhoan tk = rt.getForObject("http://localhost:9000/taikhoan/{Username}", TaiKhoan.class, params);
				String sub = "QUÊN MẬT KHẨU";
				String content = "Xin chào " + kh.getHoTen()
						+ "\n BEPSHOP gửi bạn lại thông tin đăng nhập bạn đã tạo trước đó. \n Tên tài khoản:\t"
