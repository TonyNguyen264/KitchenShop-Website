...
public class GetList {
	...
	public KhachHang getKHbyLogin(HttpServletRequest request) {
		...
		KhachHang kh = rt.getForObject("http://localhost:9000/khachhangtk/{Username}", KhachHang.class, params);
		return kh;
	}
	...
	public DonHang[] getListDHbyIDKH(int IDKH) {
		...
		ResponseEntity<DonHang[]> listg = rt.getForEntity("http://localhost:9000/donhangkh/{IDKH}", DonHang[].class,
				params);
	...
	public DonHang[] getListDHbyIDNV(int IDNV) {
		...
		ResponseEntity<DonHang[]> listg = rt.getForEntity("http://localhost:9000/donhangnv/{IDNV}", DonHang[].class,
				params);
		DonHang listdh[] = listg.getBody();
		return listdh;
	}
	...
	public Loai[] getListLoai() {
		RestTemplate rt = new RestTemplate();
		ResponseEntity<Loai[]> listloai = rt.getForEntity("http://localhost:9000/loai", Loai[].class);
	...
	public SanPham[] getListSanPham() {
		RestTemplate rt = new RestTemplate();
		ResponseEntity<SanPham[]> listsanpham = rt.getForEntity("http://localhost:9000/sanpham", SanPham[].class);
	...
	public DonHang[] getListDonHang() {
		RestTemplate rt = new RestTemplate();
		ResponseEntity<DonHang[]> list = rt.getForEntity("http://localhost:9000/donhang", DonHang[].class);
		...
		for (DonHang donHang : listdh) {
			if(donHang.getIDNV()!=0)
			{
				...
				NhanVien nv = rt.getForObject("http://localhost:9000/nhanvien/{IDNV}", NhanVien.class, params);
